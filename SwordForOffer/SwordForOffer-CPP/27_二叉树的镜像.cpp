...
// 循环处理，注意这里的测试代码是在递归之后运行，交换之后再次交换，相当于复原
void MirrorIteratively(BinaryTreeNode* pRoot) {
    if(pRoot == nullptr)
        return;
    stack<BinaryTreeNode*> stackTreeNode;// 存放节点
    stackTreeNode.push(pRoot);// 首先将根节点放入其中

    while(stackTreeNode.size() > 0) {
        BinaryTreeNode* pNode = stackTreeNode.top();// 将栈顶元素出栈
        stackTreeNode.pop();
